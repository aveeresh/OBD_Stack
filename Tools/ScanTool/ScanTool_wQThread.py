...
class Window(QMainWindow, Ui_MainWindow):
    ...
    def InitDashboard(self):
        ...
        DefaultReqId = self.JsonData['default_tester_request']['request_id']
        DefaultReqData = self.JsonData['default_tester_request']['request_data']
    ...
    def SetupPIDData(self):
        ...
        self.LE_Mode_01_RawValList = [
            self.lineEdit_RawVal_Mode_01_PID_04,
            self.lineEdit_RawVal_Mode_01_PID_05,
            self.lineEdit_RawVal_Mode_01_PID_0C,
            self.lineEdit_RawVal_Mode_01_PID_22,
            self.lineEdit_RawVal_Mode_01_PID_2C,
            self.lineEdit_RawVal_Mode_01_PID_31
        ]
        self.LE_Mode_01_PhyValList = [
            self.lineEdit_PhyVal_Mode_01_PID_04,
            self.lineEdit_PhyVal_Mode_01_PID_05,
            self.lineEdit_PhyVal_Mode_01_PID_0C,
            self.lineEdit_PhyVal_Mode_01_PID_22,
            self.lineEdit_PhyVal_Mode_01_PID_2C,
            self.lineEdit_PhyVal_Mode_01_PID_31,
        ]
        ...
        #Mode $09 data
        self.Mode_09_PIDList = [0x01, 0x02, 0x04]
        self.LE_Mode_09_RawValList = [
            self.lineEdit_RawVal_Mode_09_PID_01,
            self.lineEdit_RawVal_Mode_09_PID_02,
            self.lineEdit_RawVal_Mode_09_PID_04,
        ]
        self.LE_Mode_09_PhyValList = [
            self.lineEdit_PhyVal_Mode_09_PID_01,
            self.lineEdit_PhyVal_Mode_09_PID_02,
            self.lineEdit_PhyVal_Mode_09_PID_04,
        ]
    ...
    def ConnectSignalsSlots(self):
        ...
        self.PB_SendReq.clicked.connect(self.OnOBDRequestBtnClick)
    ...
    def UpdateSuppPIDDisplay(self):
        ...
        #end tester request
        RespMsg = self.SendTesterRequest(CanReqId=0x7E0, CanReqData=[0x02, 0x01, PID])
    ...
    def ProcessMode_01_Request(self):
        for PID in self.Mode_01_PIDList:
            #print(PID)
            self.TesterReqType = "MODE01"
            RespMsg = self.SendTesterRequest(ReqId=0x7E0, ReqData=[0x02, 0x01, PID])
            
    def ProcessMode_03_Request(self):
        RespMsg = self.SendTesterRequest(ReqId=0x7E0, ReqData=[0x01, 0x03])
    ...
    def ProcessMode_09_Request(self):
        for PID in self.Mode_09_PIDList:
            #print(PID)

            RespMsg = self.SendTesterRequest(ReqId=0x7E0, ReqData=[0x02, 0x09, PID])
            
            PIDIndexVal = self.Mode_09_PIDList.index(PID)
            RawValHandle = self.LE_Mode_09_RawValList[PIDIndexVal]
            PhyValHandle = self.LE_Mode_09_PhyValList[PIDIndexVal]
        
            if RespMsg is not None:
                CanData = list(RespMsg.data)
                if self.Mode_09_PIDDetails[PIDIndexVal]['num_bytes']==1:
                    RawVal = CanData[3]
                    if self.Mode_09_PIDDetails[PIDIndexVal]['type']=='number':
                        PhyVal = RawVal
                elif self.Mode_09_PIDDetails[PIDIndexVal]['num_bytes']==2:
                    RawVal = CanData[3]*256+CanData[4]
                
                RawValHandle.setText(str(hex(RawVal)))
                PhyValHandle.setText(str(hex(PhyVal)))
            else:
                RawValHandle.setText('TIMEOUT')
                PhyValHandle.setText('TIMEOUT')

    def SendTesterRequest(ReqId, ReqData):
        if self.ConnectStatus==True:
            self.thread = TesterRequest(CanHandle=self.CanHandle, CanId=ReqId, CanData=ReqData)
            self.thread.result_ready.connect(self.OnTesterResponse)
            self.thread.start()            
    
    def ProcessMode_XX_Resp(self, RespMsg):
        Mode = RespMsg.data[1]
        if Mode==0x01:            
            PIDIndexVal = self.Mode_01_PIDList.index(PID)
            RawValHandle = self.LE_Mode_01_RawValList[PIDIndexVal]
            PhyValHandle = self.LE_Mode_01_PhyValList[PIDIndexVal]
        
            if RespMsg is not None:
                CanData = list(RespMsg.data)
                if self.Mode_01_PIDDetails[PIDIndexVal]['num_bytes']==1:
                    RawVal = CanData[3]
                elif self.Mode_01_PIDDetails[PIDIndexVal]['num_bytes']==2:
                    RawVal = CanData[3]*256+CanData[4]
                
                #Convert raw value to physical value
                PhyVal = (RawVal+self.Mode_01_PIDDetails[PIDIndexVal]['offset'])/self.Mode_01_PIDDetails[PIDIndexVal]['factor']

                RawValHandle.setText('0x' + str(hex(RawVal))[2:].upper())
    ...
    def OnOBDRequestBtnClick(self):
        if self.ConnectStatus==True:
            ...
            if self.ConnectStatus==True:
                self.TesterReqType = "GENERAL"
                self.thread = TesterRequest(CanHandle=self.CanHandle, Id=ReqId, Data=ReqBytes)
                self.thread.result_ready.connect(self.OnTesterResponse)
                self.thread.start()            
        else:
            self.statusbar.showMessage("CAN LOGGER - Not connected", self.MSG_DISPLAY_TIME)

    def OnTesterResponse(self, RespMsg):
        if RespMsg is not None:
            if self.TesterReqType=="GENERAL":
                self.ProcessOBDReqBtnClickResponse(RespMsg)
            else:
                self.ProcessMode_XX_Resp(RespMsg)
            
    def ProcessOBDReqBtnClickResponse(self, RespMsg):
        if RespMsg is not None:
...
class TesterRequest(QThread):
    result_ready = pyqtSignal(can.Message)
    P2_TIME_MS   = 5

    def __init__(self, CanHandle=None, Id=0x7E0, Data=[0x00], TOVal_ms=P2_TIME_MS):
        super().__init__()
        self.CanHandle = CanHandle
        self.CanId     = Id
        self.CanData   = Data
        self.Timeout   = TOVal_ms
        self.RxStatus  = False
        
    def run(self):
        self.TesterRequestWithTimeout()
    
    def TesterRequestWithTimeout(self):
        ReqMsg = self.CanHandle.CreateMessage(CanId=self.CanId, CanData=self.CanData)
        ...
        if RespMsg==None:
            RespMsg = self.CanHandle.CreateMessage(CanId=None, CanData=None)
        
        self.result_ready.emit(RespMsg)
